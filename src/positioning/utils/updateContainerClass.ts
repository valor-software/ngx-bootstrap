...
export function updateContainerClass(data: Data, renderer?: Renderer2): void {
  ...
  if (data.placementAuto) {
    ...
    containerClass = containerClass + ' ' + checkMargins(data.placement);
  ...
  containerClass = containerClass.replace(/left|right|top|bottom|end|start/g, `${data.placement.split(' ')[0]}`);
