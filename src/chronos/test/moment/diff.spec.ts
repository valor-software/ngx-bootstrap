...
function equal(_assert, a, b, message) {
  _assert.ok(Math.abs(a - b) < 0.00000001, '(' + a + ' === ' + b + ') ' + message);
}
...
describe('diff', () => {
  ...
  it('month diffs', function () {
    ...
    assert.equal(moment([2012, 0, 31]).diff([2012, 1, 29], 'months', true), -1, 'Jan 31 to Feb 29 should be 1 month');
