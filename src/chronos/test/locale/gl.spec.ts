...
describe('locale: gl', () => {
  ...
  it('parse', function () {
    var _tests = 'xaneiro xan._febreiro feb._marzo mar._abril abr._maio mai._xuño xuñ._xullo xul._agosto ago._setembro set._outubro out._novembro nov._decembro dec.'.split('_'),
      i;
  ...
  it('suffix', function () {
    assert.equal(moment(30000).from(0), 'en uns segundos', 'prefix');
    assert.equal(moment(0).from(30000), 'fai uns segundos', 'suffix');
  });

  it('now from now', function () {
    assert.equal(moment().fromNow(), 'fai uns segundos', 'now from now should display as in the past');
  });

  it('fromNow', function () {
    assert.equal(moment().add({ s: 30 }).fromNow(), 'en uns segundos', 'en uns segundos');
  ...
  it('calendar day', function () {
    ...
    assert.equal(moment(a).add({ d: 1 }).calendar(), 'mañan ás 12:00', 'tomorrow at the same time');
    assert.equal(moment(a).add({ d: 1, h: -1 }).calendar(), 'mañan ás 11:00', 'tomorrow minus 1 hour');
    assert.equal(moment(a).subtract({ h: 1 }).calendar(), 'hoxe ás 11:00', 'Now minus 1 hour');
    assert.equal(moment(a).subtract({ d: 1 }).calendar(), 'onte ás 12:00', 'yesterday at the same time');
  });
  ...
  it('calendar next week', function () {
    ...
    for (i = 2; i < 7; i++) {
      m = moment().add({ d: i });
      assert.equal(m.calendar(), m.format('dddd [á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today + ' + i + ' days current time');
      m.hours(0).minutes(0).seconds(0).milliseconds(0);
      assert.equal(m.calendar(), m.format('dddd [á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today + ' + i + ' days beginning of day');
      m.hours(23).minutes(59).seconds(59).milliseconds(999);
      assert.equal(m.calendar(), m.format('dddd [á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today + ' + i + ' days end of day');
    }
  });
  ...
  it('calendar last week', function () {
    ...
    for (i = 2; i < 7; i++) {
      ...
      assert.equal(m.calendar(), m.format('[o] dddd [pasado á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today - ' + i + ' days current time');
      m.hours(0).minutes(0).seconds(0).milliseconds(0);
      assert.equal(m.calendar(), m.format('[o] dddd [pasado á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today - ' + i + ' days beginning of day');
      m.hours(23).minutes(59).seconds(59).milliseconds(999);
      assert.equal(m.calendar(), m.format('[o] dddd [pasado á' + ((m.hours() !== 1) ? 's' : '') + '] LT'), 'Today - ' + i + ' days end of day');
    }
  });
